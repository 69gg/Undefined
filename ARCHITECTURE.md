...
-   **本地 Prompt 插槽**：两份主 Prompt 提供 P0/P1/P2/P3/summary 稳定插槽；`[prompt.file_includes]` 指向的 UTF-8 文件在每次请求构建时直接读取，支持内容与配置路径热更新。推荐的 `config/prompts/*.local.*` 同时从 Git 与构建产物排除。
