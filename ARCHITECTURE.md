...
9. **部署编排层**（`deploy/`，不参与运行时）：`uv run deploy` 把本体与 NapCat、可选自托管服务（SearXNG / Firecrawl / lxmusic2api）编排成一份 Docker Compose。`catalog.py` 是服务、端口与默认值的唯一事实来源（容器内端口固定、宿主端口可覆盖），`generate.py` 由包内 compose 片段与配置模板渲染出 `deploy/` 下的运行态，`config_patch.py` 只对 `config.toml` 做最小差异写入并把原文件备份到 `deploy/backup/`（**注意它是整份重排后原子替换，不是原地编辑**：注释会尽量保留，但不要指望行号与手工排版不变），`state.py` 定义 `deploy/` 运行态布局与 `STATE.json`（端口、绑定地址、服务选择跨次复用的来源），`images.py` 集中管理上游镜像 pin，`nagaagent.py` 负责子模块就绪与「只开问答能力、绝不改写 `[naga]`」的开关写入，`docker_cli.py` 是唯一的进程外调用边界，`runner.py` 实现 up/down/status/logs，`cli.py` 负责参数解析与交互向导。注意 `deploy/` 运行态目录不入库，模板随 wheel 分发；详见 [容器化一键部署](docs/docker-deploy.md)。
