...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        chat_model = ChatModelConfig(
            ...
            thinking_enabled=os.getenv("CHAT_MODEL_THINKING_ENABLED", "false").lower() == "true",
            thinking_budget_tokens=int(os.getenv("CHAT_MODEL_THINKING_BUDGET_TOKENS", "20000")),
        )
        ...
        vision_model = VisionModelConfig(
            ...
            thinking_enabled=os.getenv("VISION_MODEL_THINKING_ENABLED", "false").lower() == "true",
            thinking_budget_tokens=int(os.getenv("VISION_MODEL_THINKING_BUDGET_TOKENS", "20000")),
        )
        ...
        if security_model_api_url and security_model_api_key and security_model_name:
            ...
            security_model = SecurityModelConfig(
                ...
                thinking_enabled=os.getenv("SECURITY_MODEL_THINKING_ENABLED", "false").lower() == "true",
                thinking_budget_tokens=int(os.getenv("SECURITY_MODEL_THINKING_BUDGET_TOKENS", "0")),
            )
        else:
            # 如果没有配置，则使用对话模型作为后备
        ...
        if admin_qq_str:
            try:
                env_admins = [int(qq.strip()) for qq in admin_qq_str.split(",") if qq.strip()]
            except ValueError:
        ...
        log_max_size = int(os.getenv("LOG_MAX_SIZE_MB", "10")) * 1024 * 1024  # 转换为字节
