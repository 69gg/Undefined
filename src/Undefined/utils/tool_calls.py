...
_JSON_FUNCTION_MARKER_RE = re.compile(r'\{\s*"function"\s*:', re.DOTALL)
...
_FUNCTION_TOOL_ENVELOPE_KEYS = frozenset({"function", "parameters"})
...
def _has_json_tool_marker(text: str) -> bool:
    return bool(
        ...
        or (
            _JSON_FUNCTION_MARKER_RE.search(text)
            and _JSON_PARAMETERS_MARKER_RE.search(text)
        )
    )
