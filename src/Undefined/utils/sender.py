...
class MessageSender:
    ...
    async def send_group_message(
        ...
        for line in lines:
            ...
            if current_length + line_length > MAX_MESSAGE_LENGTH and current_chunk:
                chunk_text = "\n".join(current_chunk)
                await self.onebot.send_group_message(group_id, message_to_segments(chunk_text))
                current_chunk = []
                current_length = 0
    ...
    async def send_private_message(
        ...
        for line in lines:
            ...
            if current_length + line_length > MAX_MESSAGE_LENGTH and current_chunk:
                chunk_text = "\n".join(current_chunk)
                await self.onebot.send_private_message(user_id, message_to_segments(chunk_text))
                current_chunk = []
                current_length = 0
