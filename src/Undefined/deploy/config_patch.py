...
**取值不会被动到**：只把目标键的当前值替换成新值——复用 WebUI 那套
``apply_patch`` + ``render_toml`` + 注释映射，不自己造 TOML 写入。

注意渲染特性（不要对外宣称"文件原样保留"）：输出会按示例文件的键序与注释映射
**整份重排**，并且只有 ``parse_comment_map`` 能识别的注释块会被保留——键上方的
``# zh:`` / ``# en:`` 块会保留，但双语块里没有 ``zh:``/``en:`` 前缀的续行、以及
不依附任何键的独立说明块会丢失，键之间的空行也会被规整。写盘前有备份。
