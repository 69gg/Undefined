...
6. `observations` 代表当前输入批次提取到的一条新记忆（可能是多条中的一条）；若本轮包含 MessageBatcher 合并的多条消息，必须结合整批消息保证可追溯性
7. 若原文已显式出现实体标识（如 `昵称(数字ID)`、`用户123456`、`QQ:123456`），必须保留该数字ID；禁止擅自替换成 `sender_id` 或其他ID
8. 可参考”当前输入批次原文”和”最近消息参考”做实体消歧；当 `observations` 与参考上下文冲突时，以可验证且更具体的信息为准
