...
- 补齐完整消息语义。微信入站内容保留 Markdown 和 `<`、`>`、`&` 等字符的字面含义，混合文本与媒体在全量预检后按原始顺序发送，本地媒体路径不会泄露到正文；Bot 可通过统一工具把音频附件显式发送为 QQ 或微信原生语音，同时保留 WAV 等普通文件附件的下载语义；引用消息支持接收、历史恢复和同路由原生回复，转发、附件登记及历史记录也遵循现有消息体系的边界。
