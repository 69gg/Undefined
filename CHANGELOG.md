...
- 新增 WebUI Release 更新检查。鉴权成功后异步查询 `69gg/Undefined` 的最新正式 GitHub Release，不阻塞页面或主事件循环；后端提供 15 分钟缓存并合并并发请求，概览页也可随时手动检查。
- 新增更新确认对话框。检测到新版本时展示当前版本、目标版本和 Release 链接，并提供“稍后”与“拉取并重启”操作；补齐中英文文案、键盘焦点管理、移动端布局、错误与不可更新状态提示。
