...
- 新增 `[prompt.file_includes]` 主 Prompt 本地文件插槽。支持在 `p0`、`p1`、`p2`、`p3` 与 `summary` 五个稳定位置分别插入一个 UTF-8 文件；空路径表示禁用，未配置的标记会从最终 Prompt 中移除。插槽文件按请求读取且不缓存，文件内容修改会在下一次 AI 调用生效，配置路径修改沿用现有热更新链路。
