...
- 统一非正数输出上限语义。所有生成模型、后台协调任务及模型池条目的 `max_tokens` 设为 `0` 或负数时均不再发送输出 token 上限，交由服务端使用默认值；正数在 Chat / Anthropic 中发送为 `max_tokens`，在 Responses 中发送为 `max_output_tokens`。Anthropic 使用 SDK 的 `omit` 哨兵确保字段不会进入实际请求体。
