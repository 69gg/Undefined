...
- 地址与端口按模式对齐：容器内走 compose 服务名与固定的容器内端口，`--port` 覆盖的是宿主机发布端口——`[webui].port` / `[api].port` 与映射目标始终一致，`[onebot].ws_url` 不会被宿主端口污染；`host` 模式下本体自己就是监听方，端口跟随 `--port`。`--port-bind` 默认只绑 `127.0.0.1`，`0.0.0.0` 只作监听语义、不会被写成连接地址。
- 凭据自动生成并按 `deploy/.env` → `config.toml` → 随机三级取值，重跑不会把已生效的 WebUI 密码、Runtime `auth_key`、NapCat token、SearXNG `secret_key`、Firecrawl 与 lxmusic2api 凭据换掉（`changeme` 与空值视为占位符）；凭据文件权限 0600，`deploy/` 整体不入库。
...
- 同一节补充宿主环境依赖：Docker（Code Delivery Agent，可 `enabled = false` 关闭）、FFmpeg（Bilibili 音视频合并、原生语音与微信侧 SILK 编码），以及本地子进程 MCP 服务器所需的运行时（`npx` 需要 Node.js，`uvx` 随 `uv` 一同安装，无需 Node.js）。概览放在源码部署的安装步骤之后、配置环境之前，pip/uv tool 部署同样适用。
...
- 兼容 `{"tool":"end","memo":"","observations":[]}` 等平铺 JSON 工具封包；除 `tool` 外的顶层字段会按原有类型转换为工具参数，并沿用现有的原生 Tool Call 执行、权限检查与续轮回放链路。
- 保留 `arguments` 嵌套参数格式，并拒绝在同一封包中混用嵌套与平铺参数，避免参数覆盖和优先级歧义；同步补充单元测试、主 Chat 执行链回归测试与模型兼容文档。

---

## v3.10.0 音乐工具集与多模型 Tool Call 兼容

本版本新增基于 lxmusic2api 的完整音乐能力，并围绕多模型 Tool Call、消息交付、队列调度与上下文边界进行系统加固。主 AI 现在可以在不暴露底层下载作业的前提下完成歌曲检索、版本选择和音频交付；不同模型输出的多种文本 Tool Call 也能安全恢复为原生调用。
...
- 完成结果驱动的搜歌与交付链路。搜索结果使用请求内有效的紧凑 `track_ref`，减少完整 Track 在模型上下文中的重复传输；AI 根据标题、歌手、专辑、版本与可用音质灵活选择结果，在用户未指定时优先可明确识别的原唱录音室版本和最高可用音质，无法确认时再询问。`music.get_audio` 负责准备附件或链接，普通音频由 `messages.send_message` 实际发送，并在历史中保留平台、音质、名称、作者、专辑、时长、格式、大小和回退信息；空成功流会被拒绝，文件后缀按最终 MIME/音质生成，WebUI 交付也会复用原附件 UID 与音乐语义元数据。
...
- 收紧当前输入、长期记忆与群聊收件人边界。当前消息批次和当前会话元数据始终高于历史记忆、旧任务和旧地址，工具或 Agent 返回后会重新核对本轮目标；群聊提示显式标注 `bot_trigger=mention|poke|none`，不再仅凭“你”、命令语气、技术相关性或管理员身份推断消息指向 Bot，也不会把批次中一次 @ 扩散到其他独立消息。最近历史会在当前批次入队前冻结为请求级深拷贝快照，避免慢速认知检索期间的新消息穿透到旧请求并诱发复读；批次自身按消息 ID 精确剔除，兼容交错群消息与投机重发。
- 优化工具调用彩蛋。虚拟 `tool_search` 可按各类模式参与调用提示；同一轮并行调用相同工具时合并为一条并显示 `xN`，减少重复消息。同步更新配置模板、README、架构、使用说明、模型兼容、认知记忆与 Tool Search 文档，并补充相关回归测试。
- 修复两个原生客户端的前端构建依赖安全告警。将 Vite 依赖树中的 PostCSS 锁定版本更新至 `8.5.23`，同步更新 Nano ID 至 `3.3.16`，消除 source map 自动加载可能造成任意 `.map` 文件泄露的路径穿越风险；两个客户端的 `npm audit` 均恢复为零漏洞。
