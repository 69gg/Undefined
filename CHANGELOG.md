...
- 修正工具调用轮次中的空白模型响应处理：实际内容为空或仅含空白字符且没有工具调用时，按请求失败原样重试，不再追加“未调用工具”纠正消息。
- 扩展文本 Tool Call 后备解析，兼容顶层 `{"tool_calls":[{"name":"...","arguments":{...}}]}` 简写封包及其中的并行调用；数组项可选携带非空 `id`，恢复为原生调用时保留该 ID，并拒绝重复 ID。
- 加固群聊收件人判定：每条当前群消息显式标记 @/拍一拍状态，并将收件人闸门置于内容理解和工具调用之前；“你”、命令语气、技术相关性或发送者权限不再被提示词视为指向 Bot 的充分证据，批次内显式触发也不会自动扩散到其他独立消息。
