...
- 新增 `[naga].mode`（`off` / `blacklist` / `allowlist`）及 `allowed_group_ids` / `blocked_group_ids` / `allowed_private_ids` / `blocked_private_ids`，语义对齐 `[access]`。`[features].nagaagent_mode_enabled` 与 `[naga].enabled` 仍为进程总闸。
