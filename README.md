...
- **远程管理 + 多端客户端**：浏览器版 WebUI、跨平台 Console 和原生优先 Undefined Chat 共享同一套 Management / Runtime 服务，支持远程管理，并覆盖 `Windows / macOS / Linux / Android` 发布链路。
...
- 💬 **[Undefined Chat](docs/undefined-chat.md)**：原生优先 WebChat 客户端说明，包含 Runtime 真源、SSE/JSON fallback、安全存储、附件上传和 HTML 预览隔离。
