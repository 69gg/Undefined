...
- **自动化工作流**：消息（关键词 / @）、拍一拍、入退群或定时触发的轻量工作流，把工具调用、LLM 加工与发送串成一张可在 WebUI 画布编辑的流程图，命中后还能接管本轮 AI 回复；旧定时任务在启动时自动迁移。详见 [docs/automations.md](docs/automations.md)。
...
- 🔁 **[自动化工作流](docs/automations.md)**：消息 / 时间 / 入退群触发的流程图，@ 匹配、分支循环与变量传递。
