...
- **微信 ClawBot/iLink 私聊**：可将一个微信帐号映射为逻辑 QQ 身份，共享权限、私聊历史、认知记忆和模型偏好，同时用 `wechat:<QQ号>` 保持物理回复路由隔离；支持 Markdown、媒体及同一物理会话内的引用收发，并带二维码管理、未知来源隔离、高权限身份二次确认和审计。无需安装 OpenClaw 或微信插件。详见 [微信 iLink 接入](docs/wechat-ilink.md)。
