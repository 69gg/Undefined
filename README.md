...
- **同 sender 短时消息合并**：同一发送者在 `[message_batcher].window_seconds`（默认 5s）内连续发送的多条消息会被合并到同一轮 AI 调用，AI 一次性看到全部 `<message>` 块自行识别"独立请求/修正/打断"，避免老版本"画猫→改成狗"造成的重复触发与回复打架。拍一拍永远旁路立即处理；群聊已有 buffer 时新到的 @bot 也会单独立即处理；首条 @bot 进入 buffer 时整批走 mention 队列。详见 [docs/message-batching.md](docs/message-batching.md)。
