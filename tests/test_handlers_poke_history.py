...
async def test_private_poke_writes_history_and_triggers_reply() -> None:
    ...
    assert (
        private_history_call.kwargs["text_content"]
        == "测试用户(暱称)[20001(QQ号)] 拍了拍你。"
    )
    ...
    assert private_reply_call.args[1] == "测试用户(暱称)[20001(QQ号)] 拍了拍你。"
    assert private_reply_call.args[2] == []
    assert private_reply_call.kwargs["is_poke"] is True
    assert private_reply_call.kwargs["sender_name"] == "测试用户"
...
async def test_group_poke_writes_history_and_triggers_reply() -> None:
    ...
    assert group_reply_call.args[2] == "群名片(暱称)[20001(QQ号)] 拍了拍你。"
    assert group_reply_call.args[3] == []
    assert group_reply_call.kwargs["is_poke"] is True
    assert group_reply_call.kwargs["sender_name"] == "群名片"
    assert group_reply_call.kwargs["group_name"] == "测试群"
    assert group_reply_call.kwargs["sender_role"] == "admin"
    assert group_reply_call.kwargs["sender_title"] == "头衔"
