...
class TestScheduledTaskSelfInstruction:
    def test_from_dict_with_self_instruction(self) -> None:
        d: dict[str, Any] = {
            "task_id": "self-1",
            ...
            "task_name": "future_me",
            "max_executions": None,
            "self_instruction": "明天提醒我看板更新",
        }
        task = ScheduledTask.from_dict(d)
        assert task.self_instruction == "明天提醒我看板更新"

    def test_roundtrip_preserves_self_instruction(self) -> None:
        d: dict[str, Any] = {
            "task_id": "self-2",
            "tool_name": "scheduler.call_self",
            "tool_args": {"prompt": "每晚复盘"},
            "cron": "0 23 * * *",
            "target_id": None,
            "target_type": "private",
            "task_name": "nightly",
            "max_executions": 5,
            "self_instruction": "每晚复盘",
        }
        task = ScheduledTask.from_dict(d)
        restored = task.to_dict()
        assert restored["self_instruction"] == "每晚复盘"

    def test_missing_self_instruction_defaults_to_none(self) -> None:
        d: dict[str, Any] = {
            "task_id": "t6",
            "tool_name": "x",
            "tool_args": {},
        ...
        task = ScheduledTask.from_dict(d)
        assert task.self_instruction is None
