...
- 支持文本和图片消息。
- AI 回复支持 Markdown 渲染，代码块会在本地进行轻量语法高亮，覆盖常见的 JSON、Python、JavaScript / TypeScript、HTML、CSS、Shell 等代码片段。
...
- 工具 / Agent 调用块展开后会分区展示由后端脱敏截断后的输入和输出预览；Agent 内部工具、子 Agent 和发送出的正文会按后端提供的 timeline 嵌套显示，并保留各自状态、输出和耗时。Agent 内部阶段只作为对应 Agent 摘要行的当前状态展示，不单独占用一行。并发工具结束事件按实际完成时间发布，结构化预览会渲染为带颜色的键值字段。输出内容继续支持 Markdown、图片和文件卡片渲染，但 WebUI 会禁用原始 HTML、过滤不安全链接协议，并保留适合移动端的单行工具摘要。
