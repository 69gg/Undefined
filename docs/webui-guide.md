...
- 支持文本和图片消息。
- AI 回复支持 Markdown 渲染。
- 默认加载最新 50 条消息并滚动到底部；向上滚动到顶部会按页懒加载更早历史，并保持滚动位置，避免一次性恢复大量工具块造成卡顿。右上角“自动滚动到底部”开关默认开启并保存在浏览器本地，关闭后 AI 回复、工具状态和阶段刷新不会打断当前位置；首次加载和主动发送新消息仍会定位到底部。
- 对话由 WebChat job 执行。刷新页面、关闭页面或网络短暂中断时，后端任务继续运行，前端会用 `job_id + seq` 轮询查询增量事件和当前阶段快照自动续接；SSE 仅作为兼容方式保留，WebUI 不依赖长连接。
...
- 工具 / Agent 调用块展开后会分区展示脱敏截断后的输入和输出预览；Agent 内部阶段、工具、子 Agent 和发送出的正文会按后端提供的 timeline 嵌套显示，并保留各自状态、输出和耗时。并发工具结束事件按实际完成时间发布，结构化预览会渲染为带颜色的键值字段，输出内容继续支持 Markdown、图片和文件卡片渲染。
