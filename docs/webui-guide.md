...
- **全屏画布**：左侧节点盘，中间点选出点再点目标连线，右侧结构化检查器。空白 LLM 的工具 / 工具集 / Agent 用搜索点选。工具与 LLM 节点可勾选「存储为变量」并填写名称，下游用 `{{名称}}` 引用输出。分支带 case 锚点，循环以分组框表示；JSON 仅作为高级排障。
- **Start 检查器**：场景多选（群 / QQ 私聊 / 微信）、群号与 QQ、@ 条款、剩余文本、`pass_text`、clock / 星期、冷却与时间类字段。
