...
- 支持文本和图片消息。
- AI 回复支持 Markdown 渲染。
- 默认加载最新消息并滚动到底部；向上滚动到顶部会懒加载更早历史，并保持滚动位置。右上角“自动滚动到底部”开关默认开启并保存在浏览器本地，关闭后 AI 回复、工具状态和阶段刷新不会打断当前位置；首次加载和主动发送新消息仍会定位到底部。
- 流式对话由 WebChat job 执行。刷新页面、关闭页面或网络短暂中断时，后端任务继续运行，前端会按 `job_id + seq` 自动续接事件。
- 运行中的 AI 气泡会在 `AI` 标签后实时显示当前阶段和后端计算的已用时，例如构建上下文、查长期记忆、查认知记忆、等待模型、等待工具、发送消息；任务完成或失败后该阶段提示会自动收起。工具块会显示工具耗时，整轮回复总耗时会写入 WebChat 历史 metadata。
- WebChat job 事件会更新同一个 AI 气泡；AI 正文和主对话工具 / Agent 调用按事件时序显示，例如工具调用、正文回复、结束工具会依次出现在同一气泡内。
- 工具 / Agent 调用块展开后会分区展示脱敏截断后的输入和输出预览；Agent 内部调用的工具、子 Agent 和发送出的正文会按后端提供的 timeline 嵌套显示，并保留各自状态、输出和耗时。并发工具结束事件按实际完成时间发布，结构化预览会渲染为带颜色的键值字段，输出内容继续支持 Markdown、图片和文件卡片渲染。
