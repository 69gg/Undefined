...
- 支持文本和图片消息。
- AI 回复支持 Markdown 渲染，Markdown 内的常见 HTML 片段会经过 WebUI 白名单净化后自动渲染；脚本、事件属性、危险协议和危险样式会被剥离。代码块会在本地进行轻量语法高亮，覆盖常见的 JSON、Python、JavaScript / TypeScript、HTML、CSS、Shell 等代码片段。
