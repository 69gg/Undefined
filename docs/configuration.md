...
合并转发会复用同一注册表登记为 `forward_...` UID，并在实时 AI 输入中显示为 `<forward uid="..."/>`。历史记录仍保留递归展开后的文本；需要查看实时上下文里的转发内容时，AI 会调用 `messages.get_forward_msg` 按层读取，内层合并转发会继续分配新的 `forward_...` UID。工具读取时会优先使用 `data/cache/forward_snapshots/` 下的本地快照，缺失时再回源 OneBot；如果协议端无法二次读取内层转发，会返回明确诊断和可见原始字段。
