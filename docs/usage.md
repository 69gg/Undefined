...
历史统计只使用带有效时间戳的消息；缺失、非法或越界的时间会被跳过，不会替换为当前时间后计入消息数、活跃天数和排名。
...
> *"每天早上 9:00 总结一下昨天群里的重点发到本群。"*
> *"有人 @ 我并提到‘签到’时，帮我回一句打卡成功。"*
> *"新人入群时自动发一段欢迎语。"*

旧 Crontab 定时任务会在启动时一次性转换为新格式。触发器、节点类型与变量系统的完整说明见 [自动化](automations.md)。
