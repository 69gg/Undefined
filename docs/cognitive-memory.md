...
- **认知记忆**（`end.observations` + `cognitive.*`）：核心层，AI 在每轮对话中主动观察并提取用户/群聊事实及有价值的自身行为（`observations`），经后台史官异步改写为绝对化事件并存入 ChromaDB 向量库，支持语义检索；当对话中出现新信息（偏好、身份、习惯等）时，史官自动合并更新 Markdown 侧写文件，下次对话时注入 prompt。
...
- `observations`：本轮值得长期留存的观察列表（0..N 条），包括用户/群聊事实和有价值的自身行为（帮谁解决了什么），严格一条一个要点；每条会独立改写与入库。
