...
源码仓库已忽略并从构建产物中排除 `config/prompts/*.local.*`。文件内容修改后下一次 AI 请求立即生效；配置路径修改后按现有配置热更新间隔生效。文件缺失或读取失败时会记录警告并跳过该插槽，不会阻止请求。完整配置与插槽位置见[配置说明](configuration.md#4112-promptfile_includes-主-prompt-本地文件插槽)。
...
只需要局部补充主 Prompt 时，优先使用上文的 `[prompt.file_includes]`：它直接读取运行目录中的本地文件并支持下一次请求热更新，也不会复制整份默认 Prompt。
