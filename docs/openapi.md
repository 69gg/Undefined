...
| `skills` | `object` | 技能统计，包含 `tools`、`toolsets`、`agents`、`auto_pipelines`、`commands`、`anthropic_skills` 子对象 |
...
`skills` 下各分类均提供轻量摘要：`tools` 是当前可调用工具总表，`toolsets` 单独拆出 `skills/toolsets/` 下的工具集工具，`agents` 对应 `skills/agents/`，`auto_pipelines` 对应 `skills/auto_pipeline/`，`commands` 对应 `skills/commands/`，`anthropic_skills` 对应全局 Anthropic Skills。常规注册表子对象结构：
...
`toolsets` 额外包含 `categories[]`，用于按工具集类别汇总；`commands` 额外包含 `aliases` 与 `subcommands` 总数；`auto_pipelines` 额外包含 `hot_reload`，用于观察自动管线热重载 watcher 是否正在运行。
