...
说明：`thinking_param_enabled` 默认开启，只控制由 `thinking_enabled` 自动生成的顶层 `thinking`，不影响 effort 或调用方显式覆盖。`reasoning_content_replay` 默认开启；关闭后不会向上游发送明文、summary、签名或加密推理材料。`responses_tool_choice_compat` 与 `responses_force_stateless_replay` 通常保持 `false`，仅在 `openai.responses` 兼容网关不支持官方对象型工具选择或状态续轮时尝试开启。
