...
`runtime/chat/history` 代理会透传 Runtime API 返回的 WebChat 展示元数据。Bot 历史项可包含 `webchat.events`，用于客户端刷新后恢复工具 / Agent 展示块；该元数据不用于 AI 上下文。删除历史时，如果 Runtime 侧仍有运行中或收尾落盘中的 WebChat job，会透传 `409`。
