...
`runtime/chat/files` 用于 WebChat 待发送文件缓存，接收 `multipart/form-data` 的 `file` 字段，返回 `{id,name,size}`，前端随后把该 id 合并为 `CQ:file` 随同当前消息提交。`runtime/chat/jobs/{job_id}/events` 默认代理 JSON 增量查询，支持 `after=<seq>` 和 `format=json`，用于 WebUI 刷新或断线后按 `job_id + seq` 恢复；返回内容包含持久事件、当前顶层 `stage` 快照、当前 `agent_stage` 快照、`current_tool_calls` 运行中工具 / Agent 快照和 job 总耗时字段。显式请求 `Accept: text/event-stream` 时仍透传 WebChat SSE。所有 Runtime 代理端点都先校验 Management session / access token，再由服务端注入 `X-Undefined-API-Key`，浏览器不会接触 Runtime `auth_key`。`runtime/chat/history` 代理会透传 Runtime API 返回的 WebChat 展示元数据。Bot 历史项可包含 `webchat.duration_ms` 与 `webchat.events`，用于客户端刷新后恢复整轮耗时、工具 / Agent 耗时、Agent 当前状态和工具 / Agent 展示块；这部分元数据不用于 AI 上下文。工具 / Agent 输入输出预览由 Runtime 侧统一脱敏和截断。删除历史时，如果 Runtime 侧仍有运行中或收尾落盘中的 WebChat job，会透传 `409`。
