...
`runtime/chat/jobs/{job_id}/events` 会透传 WebChat SSE，包括用于实时阶段提示和后端计时展示的 `stage` 事件。`runtime/chat/history` 代理会透传 Runtime API 返回的 WebChat 展示元数据。Bot 历史项可包含 `webchat.duration_ms` 与 `webchat.events`，用于客户端刷新后恢复整轮耗时、工具耗时和工具 / Agent 展示块；阶段提示不写入历史，该元数据不用于 AI 上下文。删除历史时，如果 Runtime 侧仍有运行中或收尾落盘中的 WebChat job，会透传 `409`。
