...
也兼容将一个或多个简写调用放入顶层 `tool_calls` 数组：
...
- 成功转换后，原始文本封包不会写回对话上下文；系统会保存标准的 `assistant.tool_calls` 以及对应的 `role=tool` 结果。若同一响应还携带 Responses reasoning items 或 Anthropic `thinking` / `redacted_thinking` blocks，只移除承载文本封包的 message/text 项，签名与加密推理载体会按原顺序保留，并在下一轮与恢复出的原生函数调用一同回放。
...
空响应与“未调用工具”纠正是两条不同路径。当模型没有返回原生或可恢复的 `tool_calls`，且实际 `assistant.content` 为空、仅含空格或换行符时，该轮按 LLM 请求失败处理：系统回滚本轮消息与 transport state，使用 `[core].ai_request_max_retries` 重试同一份请求，不追加空的 assistant 消息，也不注入纠正提示。即使 `reasoning_content` 非空，只要实际内容和工具调用均为空，仍属于空响应。
...
成功恢复时，`[工具调用兼容]` 日志只记录调用数量和工具名，不记录完整参数正文。解析失败时，该日志记录失败原因和内容长度；若随后进入纠正重试，`[AI回复未调用工具]` warning 会以 `raw_content=repr(...)` 完整记录该轮原始响应。空白响应改走 `[chat.pre_tool_retry]` 请求失败日志，不会出现为“未调用工具”。达到重试上限且不再进入下一轮时，不会额外输出“即将进入下一轮重试”日志。
