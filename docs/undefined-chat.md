...
- **图片经 Tauri blob 加载**：Runtime API 有认证中间件（要求 `X-Undefined-API-Key` header），而浏览器 `<img src=远程url>` 不会携带该 header → 被拦截（401）。Undefined Chat 作为独立 Tauri 应用与 Runtime **不同源**，故图片不直接用远程 URL，而是经 `AttachmentImage` 组件按 UID 调用 `previewAttachment`（Rust 端带 auth 拉取附件字节）→ `Blob` → `URL.createObjectURL` 渲染。`AttachmentImageContext` 统一缓存 blob（LRU + 并发去重，blob URL 归 Context 管理释放），缩略图、正文 `<attachment uid/>` 图、点击查看的全屏大图复用同一 blob。文件下载同样经 Tauri（`saveAttachment`）带 auth 完成。后端附件端点（`/api/v1/chat/attachments/{uid}`、`.../preview`）支持从全局 `AttachmentRegistry` 解析命令/AI 输出的图片（限 `webui` 作用域），并把粗分类 `media_type` 规范为真正的 MIME。
