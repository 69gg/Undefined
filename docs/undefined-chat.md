...
- **命令系统**：命令面板由后端 `/api/v1/commands` 完整命令元数据驱动（命令、子命令、用法、示例、别名）。输入 `/命令 ` 进入子命令模式并展示候选；命令无子命令时显示帮助卡片。方向键导航越界自动滚动，`Tab`/`Enter` 补全当前项，`Esc` 关闭。解析、匹配、补全抽象为 `message-composer/command-context.ts` 纯函数，1:1 对齐 WebUI 逻辑并附单元测试。
