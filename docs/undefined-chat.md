...
| 移动端安全存储 | 已接入，待设备验证 | iOS 使用 Keychain；Android 使用 Keystore + AES-GCM，平台不可用时才允许显式不安全降级 |
...
| 密钥保管 | 桌面系统 keyring/Stronghold；iOS Keychain；Android Keystore | WebUI 后端注入 Runtime auth key |
...
- macOS/iOS 使用系统钥匙串，Windows 使用系统凭据存储。
- Linux 依赖 Secret Service/keyring；无可用 keyring 时，必须让用户显式确认后才允许降级到本地文件。
- Android 通过生成工程注入的 `SecretPlugin` 使用 Android Keystore + AES-GCM 保存 API Key。
- 降级模式只适合本机开发或受控环境。
...
- 后台运行中 job 恢复。
- iOS Keychain 保存/读取；Android Keystore 保存、重启后读取、删除和降级提示。
