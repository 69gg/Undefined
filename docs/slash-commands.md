...
> **⚠️ 此功能面向与 NagaAgent 对接的高级场景，普通用户不建议开启。** 需要先在 `config.toml` 中启用 `[naga]` 配置。

- **/naga \<子命令\> [参数]**
  - **说明**：NagaAgent 绑定管理。通过子命令完成绑定申请、审批、吊销和查询。
  - **前置条件**：`config.toml` 中 `features.nagaagent_mode_enabled = true`。
  ...
  **权限模型**：命令入口 `config.json` 声明 `"permission": "public"`（允许所有人触发），实际权限由 `scopes.json` 按子命令细粒度控制（详见下方"scopes.json 子命令权限"一节）。
  ...
  - **额外行为**：
    - `bind` 仅在 `naga.allowed_groups` 白名单内的群可用，其余群静默忽略。
