...
- 出站：文本、图片、文件、视频和输入中状态；超过 4000 字符的文本会自动分片。
- 明确不支持：微信出站语音、`reply_to` 引用回复、群聊。

微信没有 QQ 合并转发的等价合同。对当前微信私聊发送合并转发时，Undefined 会按节点顺序降级为普通文本和媒体。包含本地 CQ 图片/文件的视频消息会走统一附件发送层；语音 CQ 码会返回不支持错误，不会悄悄改走 QQ。
